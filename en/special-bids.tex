...
\begin{table}
	...
	\begin{center}
		\begin{tabularx}{\textwidth}{
			...
			{---}
			{%
				Once during the game after a player has played a card, but before the next player has played their card, the declarer may exclaim ``Veto!''. If possible, the played card must then be replaced with another. The veto is consumed even if it was not possible to switch the played card.
			}
			...
			{\nonTrump, Grill}
			{%
				Before the game starts, all players but the declarer sends 4 cards to the player to the right (skipping the declarer). If \emph{Mail} has been bid, the \emph{Mail}-cards are sent before the \emph{Master's Mail}-cards.
			}
